    ...
    <el-card>
      ...
      <el-tabs v-model="activeTab">
        <el-tab-pane label="剪刀信息" name="info">
          <el-table :data="scissorList" border style="width: 100%">
        ...
        <el-tab-pane label="订购记录" name="order">
          ...
          <el-table :data="orderList" border style="width: 100%">
            <el-table-column label="剪刀" width="120" sortable>
              <template #default="{ row }">
        ...
        <el-tab-pane label="使用记录" name="use">
          ...
          <el-table :data="useList" border style="width: 100%">
            ...
            <el-table-column prop="user" label="使用人" width="100" sortable />
            <el-table-column prop="quantity" label="使用数量" width="100" sortable />
            <el-table-column prop="useDate" label="使用时间" width="120" sortable />
            <el-table-column prop="remark" label="备注" />
          </el-table>
        </el-tab-pane>
...
const orderForm = ref({ scissorId: '', quantity: 1, orderDate: '', status: '未到货', remark: '' })

const showUseDialog = ref(false)
const useForm = ref({ scissorId: '', user: '', quantity: 1, useDate: '', remark: '' })
...
async function loadData() {
  try {
    scissorList.value = await scissorApi.getAll()
    orderList.value = await scissorOrderApi.getAll()
    useList.value = await scissorUseApi.getAll()
    linkList.value = await scissorLinkApi.getAll()
    // 自动计算库存
    stockList.value = await stockCalcApi.calculate('scissor')
  } catch (error) {
...
function handleAdd() {
  isEdit.value = false
  form.value = { id: '', name: '', diameter: '', wireMaterial: '', safetyStock: 0 }
